...
proto.run = async function() {
  ...
  schema = data ? data.__schema : undefined;
...
proto._findResponseKind = function(type) {
  if (type) {
...
proto._buildQuery = function(schema, type) {
  return `query { ${this.query} ${this._buildFields(schema, type)}}`;
};

proto._buildFields = function(schema, type) {
  let q = "";
  if (type.fields) {
    q += "{ ";
...
proto._runQuery = async function(gqClient, query) {
